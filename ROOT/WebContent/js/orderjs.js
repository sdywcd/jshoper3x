...
$(function(){
	...
	$('#submitvoucher').click(function(){
		...
		$.post("GetVouchersByname.action",{"vouchername":vouchername},function(data){
			if(data.svoucher){
				//跳转到登录页面
				var hidurl=$('#hidurl').val();
				window.location.href="/jshop/usercenter/login.jsp?redirecturl="+hidurl;
	...
	function InitAlipayandAddOrder(){
		...
		if(paymentid!=null){
			$.post("InitAlipayneedInfo.action",{"cartid":cartid,"paymentid":paymentid,"logisticsid":logisticsid,"addressid":deliveraddressid,"logisticswebaddress":logisticswebaddress,"customernotes":customernotes,"orderTag":"1"},function(data){
				if(data.spayment){
					alert("支付方式获取失败");
					return;
				}
