...
import com.jshop.action.mall.backstage.utils.Validate;
...
public class FunctionMAction extends BaseTAction {
	...
	@Resource
	private BaseTService<FunctionM> baseTService;
	private String id;
	...
	public String findFunctionMById() {
		if (StringUtils.isNotBlank(this.getId())) {
			bean = this.baseTService.findByPK(FunctionM.class, this.getId()
					.trim());
			this.setSucflag(true);
			return "json";
		}
	...
	public void findDefaultAllFunctionMBymoduleid() {
		...
		List<FunctionM> list =this.baseTService.findByCriteria(FunctionM.class, criterion,order);
		if (!list.isEmpty()) {
			total = list.size();
	...
	public void findAllFunctionM() {
		List<FunctionM> list = this.baseTService.findAll(FunctionM.class);
		if (!list.isEmpty()) {
			total = list.size();
			this.ProcessFunctionMList(list);
		}
	...
	public String delFunctionM() {
		if (StringUtils.isNotBlank(this.getId())) {
			...
			for(String s:strs){
				FunctionM fm=this.baseTService.findByPK(FunctionM.class, s);
				if(fm!=null){
	...
	public String findAllFunction() {
		beanlist = this.baseTService.findAll(FunctionM.class);
		return "json";
	}
