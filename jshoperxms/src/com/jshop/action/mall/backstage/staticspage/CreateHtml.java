package com.jshop.action.mall.backstage.staticspage;
...
public class CreateHtml extends BaseTAction {
...
//		map.put(FreeMarkervariable.JSHOPBASICINFO, this.getDataCollectionTAction().findJshopbasicInfo(BaseEnums.DataShowState.SHOW.getState(),BaseEnums.JshopOpenState.OPEN.getOpenstate()));
//		map.put(FreeMarkervariable.SITENAVIGATIONLIST,this.getDataCollectionTAction().findSiteNavigation(BaseEnums.SiteNavigationState.SHOW.getVisible()));
//		map.put(FreeMarkervariable.FOOTCATEGORY,this.getDataCollectionTAction().findFooterCateogyrT(BaseEnums.CategoryGrade.FIRST.getState(),BaseEnums.DataUsingState.USING.getState()));
//		map.put(FreeMarkervariable.FOOTERATRICLE,this.getDataCollectionTAction().findFooterArticle(BaseEnums.DataShowState.SHOW.getState()));
//
//	}
//	
//	/**
...
//			//获取此该商品的同级分类
//			map.put(FreeMarkervariable.SECONDGOODSCATEGORY, this.getDataCollectionTAction().findSecondGoodsCategoryT(gt.getNavid(),BaseEnums.DataUsingState.USING.getState()));
//			//获取此商品顶级分类的热销商品集合
...
//		List<GoodsT> glist = this.getDataCollectionTAction().findAllGoodsT(BaseEnums.GoodsState.SALE.getState());
//		putBaseInfoToMap();
...
//		try{
//			List<ArticleT>list=this.getDataCollectionTAction().findAllArticleT(BaseEnums.DataShowState.SHOW.getState());
//			putBaseInfoToMap();
//			for (Iterator<ArticleT> it = list.iterator(); it.hasNext();) {
//				ArticleT at = (ArticleT) it.next();
//				map.put(FreeMarkervariable.ARTICLE, at);
...
//		List<ArticleT> alist = this.getDataCollectionTAction().findAllArticleT(BaseEnums.DataShowState.SHOW.getState());
//		if (!alist.isEmpty()) {
...
//		String salestate =BaseEnums.GoodsState.SALE.getState();//上架商品
//		List<GoodsCategoryT> gclist = this.getDataCollectionTAction().findAllGoodsCategoryTByState(BaseEnums.DataShowState.SHOW.getState());//读取所有启用的商品分类
//		if (!gclist.isEmpty()) {
...
//						map.put(FreeMarkervariable.SECONDGOODSCATEGORY, this.getDataCollectionTAction().findSecondGoodsCategoryT(gct.getGoodsCategoryTid(),BaseEnums.DataUsingState.USING.getState()));//获取此分类下的二级分类
//						//获取此商品顶级分类的热销商品集合
...
//		List<ArticleCategoryT>list= this.getDataCollectionTAction().findFooterCateogyrT(BaseEnums.CategoryGrade.FIRST.getState(),BaseEnums.DataShowState.SHOW.getState());
